...
async function listiraamatud() {
    ...
    for (var raamat of responseData.raamatud) {
        var raamatWithoutExtension = raamat.split('.')[0];
        resultElement.innerHTML += '<a href="http://localhost:5000/raamatud/' + raamatWithoutExtension + '"  download="' + raamat + '.txt" >' + raamat + "</a> " +
            '<a href="#" onclick="deleteObject(\'http://localhost:5000/raamatud/' + raamatWithoutExtension + '\')" > [kustuta]</a>' +
            "<br />";
    }
